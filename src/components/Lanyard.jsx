...
function Band({
  ...
  const texture = useMemo(() => {
    ...
    // Komposit tekstur tali di atas warna dasar solid supaya talinya tidak
    // transparan/tembus (background PNG biasanya transparan).
    const img = rawTexture.image;
    const canvas = document.createElement('canvas');
    canvas.width = img ? img.width : 256;
    canvas.height = img ? img.height : 256;
    const ctx = canvas.getContext('2d');
    if (!ctx) return rawTexture;
    // Warna dasar strap (solid, mirip warna lanyard asli)
